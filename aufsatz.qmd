...
In der folgenden Abbildung 1 sieht man die aus der Studie ergebenen Werte, welche wir im Folgenden weiter analysiert haben.
...
frequencies_decision_correct_rate_E <- table(study_data$decision_correct_rate_E, useNA = "no")
frequencies_decision_correct_rate_R <- table(study_data$decision_correct_rate_R, useNA = "no")
...
categorical_data_table <- data.frame(
  Variable = c("Decision Correct Rate E", "Decision Correct Rate R"), 
...
categorical_data_table %>%
  ...
  tab_header(
    title = "Antwortverteilung bei kategorialen Variablen"
  )

metric_data_table <- data.frame(
  Variable = c("age", "hct_all"),
  AnzahlRecords = c(records_count_age, records_count_hct_r01),
  Mittelwert = c(mean_age, mean_hct),
  Median = c(median_age, median_hct),
  Minimum = c(min_age, min_hct),
  Maximum = c(max_age, max_hct),
  Standardabweichung = c(sd_age, sd_hct)
  )
...
ggplot(summary_stats, aes(x = group, y = mean)) +
  ...
  labs(
    title = "t-Test: KI-System und Korrektheit",
    x = "Gruppe",
    y = "Durchschnitt"
  )
